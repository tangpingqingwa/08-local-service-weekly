...
Checkout return page (`/return`) shows paid / cancelled / unknown. It does not invent a rank before the webhook (live) or fixture settle. Occupied Polar return after a raise names Polar charged only the difference, not a full rebid. A new listing return still names listed at the bid.
