...
import { LivePaymentPort } from "./live";
import {
  parseBidUsd,
  parseChargeUsd,
  PaymentError,
...
export type FakeWaffoOptions = {
  /** Default true: createCheckout returns paid and places the listing. */
  autoSettle?: boolean;
  /**
   * Default true. False matches live Waffo `/return`: open checkouts stay
...
let defaultPort: FakePaymentPort | undefined;
let testOverride: PaymentPort | undefined;
...
function settlePaidRaise(
  ...
  const weekId = checkout.listing.weekId
    ...
    : undefined;
  if (!weekId) {
    throw new PaymentError("invalid_listing", 400, "Missing weekId");
  }
...
/** In-process Waffo fixture. Completing / auto-settling writes the listing; unpaid does not. */
export class FakePaymentPort implements PaymentPort {
  ...
  reset(): void {
    this.db.exec("DELETE FROM waffo_webhook_rejections");
    this.db.exec("DELETE FROM waffo_webhook_events");
    this.db.exec("DELETE FROM waffo_checkout_events");
    this.db.exec("DELETE FROM waffo_intents");
...
export function getFakePaymentPort(db?: AppDb): FakePaymentPort {
  if (db) {
    return new FakePaymentPort(db);
  }
  if (!defaultPort) {
    defaultPort = new FakePaymentPort(getDb());
  }
  return defaultPort;
}

/** Select only the explicit fixture, waffo-test, or waffo-prod modes. */
export function getPaymentPort(db?: AppDb): PaymentPort {
  if (testOverride) {
    return testOverride;
  }
  const mode = requireWaffoMode(process.env);
  if (mode === "fixture") {
    validateWaffoConfiguration(process.env);
    return getFakePaymentPort(db);
  }
  if (mode === "waffo-test" || mode === "waffo-prod") {
    validateWaffoConfiguration(process.env, { databaseInjected: Boolean(db) });
    return new LivePaymentPort({ db: db ?? getDb() });
  }
  throw new PaymentError("waffo_mode_required", 503, "unsupported Waffo mode");
}

export function setPaymentPortForTests(port?: PaymentPort): void {
  testOverride = port;
}

export function resetPaymentFixture(): void {
  defaultPort?.reset();
